...
---@field fromInventory? string|number
---@field toInventory? string|number
...
local function resolveInventoryForSwap(playerInventory, invType, requestedId)
    ...
    if invType == 'backpack' then
        ...
        local container = Inventory(slotData.metadata.container) or Inventory.GetContainerFromSlot(playerInventory, slotIndex)
        return container
    end
    ...
    if requestedId then
        inventory = Inventory(requestedId)
    else
        inventory = Inventory(playerInventory.open)
    end
    ...
    elseif inventory.type == 'drop' then
        local dropData = Inventory.Drops[inventory.id]

        if dropData and dropData.coords and playerInventory.player and playerInventory.player.ped then
            local coords = GetEntityCoords(playerInventory.player.ped)
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
    ...
    local fromInventory = resolveInventoryForSwap(playerInventory, data.fromType, data.fromInventory)
    local toInventory = resolveInventoryForSwap(playerInventory, data.toType, data.toInventory)
                    ...
                    if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
                        ...
                        if containerItem then
                            ...
                            if (whitelist and not whitelist[checkItem]) or (blacklist and blacklist[checkItem]) then
                                return false, 'container_restriction'
                            end

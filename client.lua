...
local currentCraftingStorageId = nil  -- Track crafting storage inventory ID
...
function client.openInventory(inv, data)
    ...
    elseif inv == 'crafting' then
		...
		distance = coords and shared.target == 'ox_target' and right.zones[data.index].distance or 2
            ...
            -- Store crafting inventory from server response
            if left.craftingInventory then
                left.craftingInventory = left.craftingInventory
                currentCraftingStorageId = left.craftingInventory.id
            end
        end
    ...
    SendNUIMessage({
        ...
        data = {
            ...
            backpackInventory = left.backpackInventory,
            craftingInventory = left.craftingInventory,
        }
    })
            ...
            elseif data.component then
                ...
                for componentIndex = 1, #weaponComponents do
                    if componentType == Items[weaponComponents[componentIndex]].type then
                        return lib.notify({ id = 'component_slot_occupied', ype = 'error', escription = locale('component_slot_occupied', componentType) })
                    end
                end
                ...
                for i = 1, #components do
                    ...
                    if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
                        if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							lib.notify({ id = 'component_has', type = 'error', escription = locale('component_has', label) })
                        else
...
local function updateInventory(data, weight)
    ...
    for i = 1, #data do
        ...
        if not v.inventory or v.inventory == cache.serverId then
            ...
            if currentCraftingStorageId and currentCraftingStorageId == inventoryId then
                v.inventory = 'crafting_storage'
            elseif currentInventory and currentInventory.id == inventoryId then
                v.inventory = currentInventory.type
            elseif shared.backpackSlot and PlayerData.inventory[shared.backpackSlot] and  PlayerData.inventory[shared.backpackSlot].metadata and  PlayerData.inventory[shared.backpackSlot].metadata.container == inventoryId then
                v.inventory = 'backpack'
            else
                -- Try to determine type from ID pattern
                if inventoryId:find('^drop%-') then
                    v.inventory = 'drop'
                elseif inventoryId:find('^glove') or inventoryId:find('^trunk') then
                    v.inventory = inventoryId:find('^glove') and 'glovebox' or 'trunk'
                elseif inventoryId:find('^dumpster%-') then
                    v.inventory = 'dumpster'
                elseif inventoryId:find('^evidence%-') then
                    v.inventory = 'policeevidence'
                else
                    local isContainer = false
                    for slot, item in pairs(PlayerData.inventory) do
                        if item and item.metadata and item.metadata.container == inventoryId then
                    ...
                    if not isContainer and currentInventory then
                        v.inventory = currentInventory.type
                    end
                end
            end
...
RegisterNetEvent('ox_inventory:inventoryReturned', function(data)
    ...
    lib.notify({ id = 'items_returned', type = 'info', description = locale('items_returned') })
...
RegisterNetEvent('ox_inventory:inventoryConfiscated', function(message)
    if source == '' then return end
    if message then lib.notify({ id = 'items_confiscated', type = 'error', description = locale('items_confiscated') }) end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
        ...
        if point.isClosest and point.currentDistance < 1.2 then
            ...
            if IsControlJustReleased(0, 38) then
                lib.callback('ox_inventory:buyLicense', 1000, function(success, message)
                    if success ~= nil then
                        lib.notify({ id = message, type = success == false and "error" or "success", description = locale(message, locale('license', point.type:gsub("%l", string.upper))) })
                    end
                end, point.invId)
            end
    ...
    if not client.disablesetupnotification then
        lib.notify({ id = 'inventory_setup', type = 'info', description = locale('inventory_setup') })
...
RegisterNUICallback('craftFromCraftingInventory', function(data, cb)
    local benchId = data.benchId
    local recipeId = data.recipeId
    local quantity = data.quantity or 1

    local success, errorMsg, responseData = lib.callback.await('ox_inventory:craftFromCraftingInventory', 200, benchId, recipeId, quantity)

    if success then
        cb({ success = true, data = responseData })
    else
        if errorMsg then 
            lib.notify({ type = 'error', description = locale(errorMsg or 'cannot_perform') }) 
...
RegisterNUICallback('startCraftQueue', function(data, cb)
    local benchId = data.benchId
    ...
    local success, errorMsg = lib.callback.await('ox_inventory:startCraftQueue', 200, benchId, queue)
...
RegisterNUICallback('getBackpackInventory', function(_, cb)
    local backpack = lib.callback.await('ox_inventory:getBackpackInventory', false)
    cb(backpack)
end)
